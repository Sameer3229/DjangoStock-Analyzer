...
from analytics.tasks import fetch_company_income_statement # Import new task

class Command(BaseCommand):
    help = 'Triggers Celery tasks for Income Statement (API 6)'

    def handle(self, *args, **kwargs):
        companies = Company.objects.all()
        count = companies.count()
        
        self.stdout.write(f"🚀 Dispatching {count} Income Statement tasks to Celery...")

        for company in companies:
            # Send to Queue
            fetch_company_income_statement.delay(company.company_id)
